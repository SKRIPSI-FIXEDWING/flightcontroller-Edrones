...
public:
    ...
    /// constructors
    Locations();
    Locations(int32_t latitude, int32_t longitude, int32_t alt_in_cm, AltFrame frame);
    Locations(const Vector3f &ekf_offset_neu);
    ...
    float get_distance(const struct Locations &loc2) const;

    // return the distance in meters in North/East/Down plane as a N/E/D vector to loc2
    Vector3f get_distance_NED(const Locations &loc2) const;

    // return the distance in meters in North/East plane as a N/E vector to loc2
    Vector2f get_distance_NE(const Locations &loc2) const; 
    ...
    int32_t get_bearing_to(const struct Locations &loc2) const;

    // check if lat and lng match. Ignore altitude and options
    bool same_latlon_as(const Locations &loc2) const;

    /*
     * convert invalid waypoint with useful data. return true if location changed
     */
    bool sanitize(const struct Locations &defaultLoc);
    ...
    // the target waypoint
    bool past_interval_finish_line(const Locations &point1, const Locations &point2) const;
     ...
     */
    float line_path_proportion(const Locations &point1, const Locations &point2) const;

    bool initialised() const { return (lat !=0 || lng != 0); }

    void point_use_bearing(Locations &to,float bearing,float distance);
private:
